...
typedef void (*lk_init_hook)(uint32_t level);
...
void lk_init_level(enum lk_init_flags flags, uint32_t start_level, uint32_t stop_level);

static inline void lk_primary_cpu_init_level(uint32_t start_level, uint32_t stop_level) {
