...
static uint32_t secondary_bootstrap_thread_count;
...
void lk_main(uint64_t arg0, uint64_t arg1, uint64_t arg2, uint64_t arg3) {
    // save the boot args
    lk_boot_args[0] = arg0;
    lk_boot_args[1] = arg1;
    lk_boot_args[2] = arg2;
...
void lk_init_secondary_cpus(uint32_t secondary_cpu_count) {
    ...
    for (uint32_t i = 0; i < secondary_cpu_count; i++) {
