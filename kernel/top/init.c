...
void lk_init_level(enum lk_init_flags required_flag, uint32_t start_level, uint32_t stop_level) {
    LTRACEF("flags %#x, start_level %#x, stop_level %#x\n",
            required_flag, start_level, stop_level);

    ASSERT(start_level > 0);
    uint32_t last_called_level = start_level - 1;
...
#if 0
void test_hook(uint32_t level) {
    LTRACEF("level %#x\n", level);
}
void test_hook1(uint32_t level) {
    LTRACEF("level %#x\n", level);
}
void test_hook1a(uint32_t level) {
    LTRACEF("level %#x\n", level);
}
void test_hook1b(uint32_t level) {
    LTRACEF("level %#x\n", level);
}
void test_hook2(uint32_t level) {
    LTRACEF("level %#x\n", level);
}
